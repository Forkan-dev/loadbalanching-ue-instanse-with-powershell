...
app.use((req, res, next) => {
    // Check if query params are present (specifically StreamerId) to verify if it's a register request
    // This avoids logging every single heartbeat or asset request
    const streamerId = req.query.StreamerId as string;
    const agentIP = req.query.AgentIP as string;
    const agentPort = req.query.AgentPort as string;
    
    // Only log and process if we have the required parameters
    if (streamerId && agentIP && agentPort) {
        Logger.info(`Request query: ${JSON.stringify(req.query)} -------->`);
        Logger.info(`Data received: ${streamerId} | ${agentIP} | ${agentPort}`);
        
        agentMap.set(streamerId, { ip: agentIP, port: parseInt(agentPort) });
        
        Logger.info(`[AutoStop] Registered ${streamerId} -> ${agentIP}:${agentPort}`);
        
        // Maps don't JSON.stringify automatically, so we convert entries to array for logging
        Logger.info(`agentMap content: ${JSON.stringify(Array.from(agentMap.entries()))}`);
    }
...
if (options.serve) {
    ...
    if (options.https) {
        webserverOptions.httpsPort = Number(options.https_port);
...
// Auto-stop: Store agent info from URL
const agentMap = new Map<string, { ip: string; port: number }>();
const playerStreamerMap = new Map<string, string>(); // Track player -> streamer mapping



// Track when players connect to streamers (on 'added' event)
signallingServer.playerRegistry.on('added', (playerId: string) => {
    // Wait a bit for the player to subscribe to a streamer
    setTimeout(() => {
        signallingServer.streamerRegistry.streamers.forEach((streamer, sid) => {
            Logger.info(`this is streamer ---------->: ${streamer.streamerId} : ---------- sid: ${sid}`);
            if (streamer.subscribers && streamer.subscribers.has(playerId)) {
                playerStreamerMap.set(playerId, streamer.streamerId);
                Logger.info(`[AutoStop] Tracked ${playerId} -> ${streamer.streamerId}`);
            }
        });
    }, 1000); // Wait 1 second for subscription to complete
});

// Auto-stop: Call agent when player disconnects
signallingServer.playerRegistry.on('removed', (playerId: string) => {
    Logger.info(`[AutoStop] Player removed event: ${playerId}`);
    
    const streamerId = playerStreamerMap.get(playerId);
    Logger.info(`[AutoStop] StreamerId from map: ${streamerId}`);
    
    if (!streamerId) {
        Logger.info(`[AutoStop] No streamerId found for player ${playerId}`);
        return;
    }
    
    const agent = agentMap.get(streamerId);
    Logger.info(`[AutoStop] Agent from map: ${JSON.stringify(agent)}`);

    if (!agent) {
        Logger.info(`[AutoStop] No agent found for streamer ${streamerId}`);
        return;
    }
    
    Logger.info(`[AutoStop] Player ${playerId} removed from ${streamerId}, stopping agent ${agent.ip}:${agent.port}`);
    ...
    const req = http.request({
        ...
        res.on('end', () => Logger.info(`[AutoStop] Agent response: ${data}`));
    });
    
    req.on('error', (err) => Logger.error(`[AutoStop] Error: ${err.message}`));
    req.on('timeout', () => { req.destroy(); });
    req.end();
    
    playerStreamerMap.delete(playerId);
    agentMap.delete(streamerId);
});
